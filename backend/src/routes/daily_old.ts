...
import { DailyChallengeData } from '../types.js';
import { createError } from '../middleware/errorHandler.js';

const router = express.Router();

// Simple in-memory storage for daily challenges (use database in production)
const dailyChallenges = new Map<string, DailyChallengeData>();
...
async function generateDailyChallenge(date: string): Promise<DailyChallengeData> {
  try {
    ...
    const settings = {
      ...
      totalRounds: 5,
      ratings: ['safe', 'questionable'] as const,
      minUpvotes: 100,
      customCriteria: ''
    };
    ...
    const challenge: DailyChallengeData = {
      ...
      settings: {
        ...settings,
        ratings: [...settings.ratings] as any
      },
      participants: 0,
      topScore: 0
    };
...
function generateDailyTags(seed: number): string {
  const tagPools = [
