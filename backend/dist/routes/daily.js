...
async function getBlacklistedTags() {
    ...
    try {
        ...
        DAILY_BLACKLIST = result.rows.map((row) => row.tag);
...
async function getDailyChallenge(date) {
    const result = await db.query('SELECT * FROM daily_challenges WHERE date = $1', [date]);
 ...
 */
async function passesBlacklist(post) {
 ...
 */
async function saveDailyChallenge(date, posts) {
 ...
 * Check if player already submitted for this date
 */
async function checkPlayerSubmission(date, playerName) {
    const result = await db.query('SELECT 1 FROM daily_results WHERE date = $1 AND player_name = $2', [date, playerName]);
