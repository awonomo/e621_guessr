...
function buildE621Query(params) {
    ...
    if (params.ratings && params.ratings.length > 0) {
        if (params.ratings.length === 1) {
            ...
            const excluded = allRatings.find(r => !params.ratings.includes(r));
            if (excluded) {
                const ratingMap = {
                    safe: '-rating:s',
                    questionable: '-rating:q',
...
router.get('/', async (req, res, next) => {
    try {
        ...
        let ratings;
        if (query.ratings) {
            ratings = query.ratings.split(',').filter(r => ['safe', 'questionable', 'explicit'].includes(r));
        }
        // Build API query
