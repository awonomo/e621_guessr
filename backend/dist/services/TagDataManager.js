...
export class TagDataManager {
     ...
     */
    async downloadAndParseTags(url) {
     ...
     */
    async downloadAndParseAliases(url) {
        const response = await axios.get(url, {
            responseType: 'stream',
            timeout: 300000 // 5 minutes
        });
     ...
     */
    async processAliases(aliasData) {
     ...
     */
    async logRefresh(date, tagsProcessed, aliasesProcessed, duration, status, errorMessage) {
        await db.query(`
    ...
    async findTag(query) {
        ...
        let result = await db.query('SELECT name, category, post_count, quality, manual_score FROM tags WHERE name = $1', [query.toLowerCase().trim()]);
...
//# sourceMappingURL=TagDataManager.js.map